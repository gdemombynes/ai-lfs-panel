...
def _tenure(raw: pd.DataFrame) -> tuple:
    ...
    months = months.mask(true_only(band == 1), 0.5)
    m2 = pd.to_numeric(raw["V40401"], errors="coerce")
    months = months.mask(true_only(band == 2), m2.where(m2.notna(), 6.0))
    y3 = pd.to_numeric(raw["V40402"], errors="coerce")
    months = months.mask(true_only(band == 3), 12.0 * y3.where(y3.notna(), 1.0) + 6.0)
    y4 = pd.to_numeric(raw["V40403"], errors="coerce")
    months = months.mask(true_only(band == 4), 12.0 * y4.where(y4.notna(), 2.0))
    lt12 = pd.Series(pd.NA, index=raw.index, dtype="Int8")
    lt12 = lt12.mask(true_only(band.isin([1, 2])), 1).mask(
        true_only(band.isin([3, 4])), 0
    )
...
def harmonize(
    ...
    df["educat7"] = vd3004.mask(true_only(vd3004.isin([6, 7])), 7).astype("Int8")
    ...
    lstatus = lstatus.mask(true_only(adult & (vd4002 == 1)), 1)
    lstatus = lstatus.mask(true_only(adult & (vd4002 == 2)), 2)
    lstatus = lstatus.mask(true_only(adult & (vd4001 == 2)), 3)
    ...
    nlf = nlf.mask(true_only((nlf == 3) & (df["age"] <= 29)), pd.NA)
    ...
    isic = cnae.str[:2] + "00"
    isic = isic.mask(true_only(cnae.isin(["48010", "48020", "48076", "48078"])), "4600")
    isic = isic.mask(true_only(isic == "4800"), "4700")
    isic = isic.mask(true_only(isic == "0000"), pd.NA)
    ...
    isco = isco.mask(true_only(isco.between("0200", "0512")), "0000")
    ...
    for code, col in ((1, "V40181"), (2, "V40182"), (3, "V40183")):
        ...
        lower = lower.mask(true_only(band == code), exact)
        upper = upper.mask(true_only(band == code), exact)
    lower = lower.mask(true_only(band == 4), 51)

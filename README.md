...
| Uruguay (ECH) | INE NADA, terms form then open download (`curl -k`) | three monthly CSVs stacked, implantation base merged for tenure | 2021Q3-2025Q4, 18 quarters (person-months, weight W/3) | 54/54 checks pass, within 0.09 pp of INE's monthly rates from 2022Q2 (`output/tables/validation_official_ury.csv`) |
| Bolivia (ECE) | INE ANDA catalogue, login, hand download: pooled 2015Q4-2025Q3 file plus per-quarter zips | pooled CSV filtered with DuckDB or Stata member per quarter | 2021Q1-2025Q4, 20 quarters | 60/60 checks pass against ILOSTAT's national rates: unemployment within 0.12 pp, participation 0.2-0.4 pp above the originally published series (revised weights) (`output/tables/validation_official_bol.csv`) |
